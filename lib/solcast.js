...
function getForecastForDate(date, dataJson, utcOffset) {
    ...
    const forecastsDate = dataJson.forecasts.filter(
        forecast => getMomentObj(forecast.period_end, utcOffset).date() == date,
    );
    let wattHours = 0;
    let timeMoment = null;
    ...
    for (let i = 0; i < forecastsDate.length; i++) {
        ...
        if (forecast.pv_estimate !== 0 && i > 0) {
            ...
            dateJson.watt_hours_period[newtime] =
                ((prevEstimate.pv_estimate + (forecast.pv_estimate - prevEstimate.pv_estimate) / 2) / 2) * 1000;
        } else {
